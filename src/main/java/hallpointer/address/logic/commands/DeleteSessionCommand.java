...
public class DeleteSessionCommand extends Command {
    ...
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Deletes the session associated to the member identified "
            + "by the index number used in the session list. "
            + "Parameters: "
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        for (Index index : memberIndexes) {
            ...
            if (memberToDeleteFrom.getSessions()
                    .stream().noneMatch(
        ...
        return new CommandResult(String.format(
                MESSAGE_DELETE_SESSION_SUCCESS, sessionName.toString(), memberIndexes.size())
        );
    }
